...
static void* hpux_get_convert_buffer(struct buffer *buffer, size_t size)
{
...
{
	hpux_get_convert_buffer(&format_buffer, 0);
	hpux_get_convert_buffer(&stereo_buffer, 0);
}
...
static int convert_to_16_native_endian(void **data, int length)
{
	...
	guint16 *output;
	int i;
	*data = hpux_get_convert_buffer(&format_buffer, length * 2);
	output = *data;
	for (i = 0; i < length; i++)
		*output++ = *input++ << 8;
...
int (*hpux_get_convert_func(int output, int input))(void **, int)
{
	...
	switch (input)
	{
	case AFMT_S16_BE: return NULL;
	case AFMT_S16_LE: return convert_swap_endian;
	case AFMT_U16_BE: return convert_swap_sign16;
	case AFMT_U16_LE: return convert_swap_sign_and_endian_to_native;
	case AFMT_S8: return convert_to_16_native_endian;
	case AFMT_U8: return convert_to_16_native_endian_swap_sign;
	default:
...
{
	int i;
	void *outbuf = hpux_get_convert_buffer(&stereo_buffer, length * 2);
